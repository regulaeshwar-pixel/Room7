...
const PoolMember = ({ mid, members, waterSelection, onToggle, onLongPress, isExempt, completionCount = 0, index = 0 }) => {
    ...
    return (
        <button
            ...
            className={`flex items-center gap-3 pr-5 py-2 pl-2 rounded-full border transition-all duration-500 animate-in fade-in zoom-in-95 fill-mode-both group relative overflow-hidden ${isSelected
        ...
        >
            ...
            <div className="relative z-10">
                <MemberAvatar
                    size="sm"
                    className={`w-8 h-8 text-[11px] font-black transition-transform duration-500 group-hover:scale-110 ${isSelected
                        ? '!bg-white !text-indigo-600 !ring-0'
                        : '!bg-indigo-50/50 !text-indigo-400'
                        }`}
                    member={member}
                />
                {isExempt && <div className="absolute -bottom-1 -right-1 bg-rose-500 rounded-full w-2.5 h-2.5 border-2 border-white shadow-sm" />}
            </div>

            <div className="flex flex-col items-start relative z-10">
                <span className="text-[12px] font-black tracking-tight leading-none">{member?.name}</span>
                {isSelected && <span className="text-[8px] uppercase tracking-[0.25em] font-black text-indigo-200 mt-1 animate-pulse">Active</span>}
            </div>

            {completionCount > 0 && (
                <div className={`text-[9px] font-black px-2 py-0.5 rounded-full ml-1 min-w-[18px] shadow-sm relative z-10 transition-colors duration-500 ${isSelected ? 'bg-white text-indigo-600' : 'bg-slate-100 text-slate-500'
                    }`}>
                    {completionCount}
                </div>
            )}
